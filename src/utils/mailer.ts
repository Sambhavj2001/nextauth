...
export const sendEmail = async ({email, emailType, userId}: {email: string, emailType: string, userId: number}) => {
    try {
        ...
        let message = '';
        if(emailType === "VERIFY") {
            const verifyUrl = `${process.env.DOMAIN}/verify-email?token=${hashedToken}&id=${userId}`;
            message = `Please verify your email by clicking on the following link: ${verifyUrl}`;
            await User.findByIdAndUpdate(userId, {verificationToken: hashedToken, verificationTokenExpiry: Date.now() + 3600000 }); // 1 hour expiry
        } else if(emailType === "RESET") {
            const resetUrl = `${process.env.DOMAIN}/reset-password?token=${hashedToken}&id=${userId}`;
